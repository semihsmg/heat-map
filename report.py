...
def generate_html(period: str = 'all') -> str:
    """Generate the heat map HTML for a given time period."""
    key_counts = database.get_key_counts(period)
    total_keys = database.get_total_keystrokes(period)
...
<head>
    ...
    <style>
        ...
        * {{
        ...
        .tab.active {{
            background: linear-gradient(135deg, rgba(0, 255, 170, 0.2), rgba(0, 221, 255, 0.1));
            border-color: var(--accent);
            color: var(--accent);
...
<body>
    ...
    <div class="container">
        ...
        <div class="tabs">
            <button class="tab{' active' if period == 'today' else ''}" onclick="window.location.href='?period=today'" data-period="today">Today</button>
            <button class="tab{' active' if period == 'week' else ''}" onclick="window.location.href='?period=week'" data-period="week">This Week</button>
            <button class="tab{' active' if period == 'month' else ''}" onclick="window.location.href='?period=month'" data-period="month">This Month</button>
            <button class="tab{' active' if period == 'all' else ''}" onclick="window.location.href='?period=all'" data-period="all">All Time</button>
        </div>
        ...
        <footer class="footer">
            Generated by Keyboard Heat Map
        </footer>
    </div>
...
def generate_report(period: str = 'all') -> Path:
    """Generate the heat map report and return the file path."""
    html = generate_html(period)
...
def open_report(period: str = 'all'):
